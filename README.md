...
## Описание проекта
В проекте реализован автоматизированный ML-пайплайн переобучения модели с использованием **Apache Airflow**. **DAG** включает этапы обучения, оценки качества и деплоя модели в цикле **CI/CD** в **Yandex Cloud Serverless Containers** при превышении метрики средневзвешенной **F1-score** в **0.9**. Версия модели передается через переменные окружения и используется в логике пайплайна. После успешного деплоя отправляется уведомление в **Telegram**. Скриншоты приложены ниже.
